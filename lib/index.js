...
const DEFAULTS = {
  ...
  /**
   * CLI 不返回 token 数（实测恒为 0），开启后用本地估算填充 usage，
   * 让 DSH 侧能显示 token 计量。估算为粗估，非计费依据。
   */
  estimateUsage: true,
}
...
export function apply(ctx, config = {}) {
  ...
  async function handleChat(req, res, body) {
    ...
    // 注意：一律用 stream-json。CLI 的 `-o json` 只吐一行 result（无 thinking 事件），
    // 非流式请求也走 stream-json 再在本地聚合，才能拿到思考过程。
    const baseArgs = [
      '-p',
      '-o',
      'stream-json',
    ...
    if (!wantStream) {
      try {
        ...
        // CLI 输出是 JSONL：assistant 行带 thinking/text，末尾 result 行带收尾状态。
        // 全部聚合出正文与思考，再包装成一条 OpenAI 非流式响应。
        const agg = aggregateCli(r.stdout)
        if (!agg.result) {
          sendJson(res, 502, {
            error: { message: 'CLI 返回了非 JSON 输出', raw: String(r.stdout || '').slice(0, 500) },
          })
          return
        }
        const text = agg.text || agg.resultText
        if (agg.isError || /Not logged in/i.test(text)) {
        ...
        sendJson(res, 200, {
          ...
          usage: normalizeUsage(agg.usage, promptText, text),
        })
      } catch (e) {
    ...
    let finished = false
    // 累积正文与思考，用于收尾时给出 usage（CLI 自身返回 0，走本地估算）
    let accText = ''
    let accReasoning = ''
    ...
    const finish = (errText) => {
      ...
      const done = {
        ...
        // OpenAI 流式约定：usage 只在最后一个 chunk 出现（配 stream_options.include_usage）
        usage: normalizeUsage(null, promptText, accText),
      }
    ...
    try {
      const child = streamCli(baseArgs, promptText, (line) => {
        ...
        for (const block of json.message.content) {
          ...
          // thinking 块 → reasoning_content 增量（DSH 的 pi-ai 认这个字段名）
          if (block.type === 'thinking' && typeof block.thinking === 'string' && block.thinking) {
            sawReasoning = true
            accReasoning += block.thinking
            const rc = {
              id,
              object: 'chat.completion.chunk',
              created,
      ...
      // 只有 thinking、没有正文也算有输出（否则会把「只出思考」误判成空返回）
      child.on('close', () => finish(resultError || (sawText || sawReasoning ? '' : 'CLI 未返回任何内容')))
  ...
  /**
   * 把 CLI 的 JSONL 一次性聚合。
   * - assistant 行 → 累积 thinking（reasoning）与 text（正文增量）
   * - result 行   → 收尾状态、最终正文、usage
   * 之前只取 result 行的 result 字段，导致 thinking 整段丢失、usage 恒为 0。
   */
  function aggregateCli(raw) {
    const out = {
      reasoning: '',
      text: '',
      resultText: '',
    ...
    for (const line of String(raw || '').split('\n')) {
      ...
      if (json.type === 'assistant' && json.message && Array.isArray(json.message.content)) {
        for (const block of json.message.content) {
          if (!block || typeof block.type !== 'string') continue
          if (block.type === 'thinking' && typeof block.thinking === 'string') out.reasoning += block.thinking
          else if (block.type === 'text' && typeof block.text === 'string') out.text += block.text
        }
        continue
      }
      if (json.type === 'result') {
        out.result = true
        out.resultText = typeof json.result === 'string' ? json.result : ''
        out.usage = json.usage || null
        out.isError = json.is_error === true
      }
    }
    return out
  }
  ...
  /**
   * Qoder CLI 的 usage 恒为 0（实测 input_tokens/output_tokens 全 0，modelUsage 亦然），
   * 这是 CLI 本身的限制，不是插件丢字段。DSH 需要 token 数来显示计量，
   * 因此对齐真实消费方：CLI 没给就用本地估算，并在 config 里留开关。
   * ponytail: 按字符粗估（CJK≈1 token/字，ASCII≈4 字符/token），非精确；
   *   若 CLI 日后返回真实 usage，优先用真实值，估算只在全 0 时兜底。
   */
  ...
  /** CLI usage → OpenAI usage 字段名；CLI 全 0 时回退到本地估算 */
  function normalizeUsage(usage, promptText, completionText) {
    const p = Number(usage && usage.input_tokens) || 0
    const c = Number(usage && usage.output_tokens) || 0
    if (p > 0 || c > 0) return { prompt_tokens: p, completion_tokens: c, total_tokens: p + c }
    if (!cfg.estimateUsage) return { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 }
    const ep = estimateTokens(promptText)
    const ec = estimateTokens(completionText)
    return { prompt_tokens: ep, completion_tokens: ec, total_tokens: ep + ec }
  }
